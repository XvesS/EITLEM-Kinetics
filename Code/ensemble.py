...
class Predictor(nn.Module):
    ...
    def final_stage(self, mol, pro):
        ...
        return self.out(torch.cat([mol_out, pro_out], dim=-1))
    
    def forward(self, x, pro_emb, pro_emb_batch):
        mol = F.relu(self.prej1(x))
        prot = F.relu(self.prej2(pro_emb))
        att_pro = []
        att_mol = []
...
# class MREnsemble(nn.Module):
#     def __init__(self, kcatMolDim, kmMolDim, hidden_dim, protein_dim, layer, dropout, att_layer):
#         super().__init__()
#         self.kcat = Predictor(kcatMolDim, hidden_dim, protein_dim, layer, dropout, att_layer)
#         self.km = Predictor(kmMolDim, hidden_dim, protein_dim, layer, dropout,att_layer)
#         self.o = nn.Sequential(
#             nn.Linear(2*hidden_dim, 4*hidden_dim),
#             nn.ReLU(),
#             nn.Dropout(p=dropout),
...
#     def forward(self, data):
#         kcat_out = self.kcat(data.x, data.pro_emb, data.pro_emb_batch)
#         km_out = self.km(data.y, data.pro_emb, data.pro_emb_batch)
#         return self.o(torch.cat([kcat_out,km_out], dim=-1)).squeeze(dim=-1)